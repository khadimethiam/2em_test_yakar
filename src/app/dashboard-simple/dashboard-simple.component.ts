...
export class DashboardSimpleComponent implements OnInit {
  ...
  async getMorningDataForSpecificTime() {
    try {
      const response: any = await this.http
        .get<{ humidity: string; temperature: string }>('http://localhost:3002/api/data/21h46')
        .toPromise();
  ...
  async getNoonDataForSpecificTime() {
    try {
      const response: any = await this.http
        .get<{ humidity: string; temperature: string }>('http://localhost:3002/api/data/21h47')
        .toPromise();
  ...
  async getEveningDataForSpecificTime() {
    try {
      const response: any = await this.http
        .get<{ humidity: string; temperature: string }>('http://localhost:3002/api/data/21h48')
        .toPromise();
