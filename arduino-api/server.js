...
// Connexion à MongoDB
mongoose
  .connect("mongodb://localhost:27017/tonBaseDeDonnees", {
...
async function saveDataToDB(hour) {
  ...
  try {
    ...
    console.error(
      `Erreur lors de l'enregistrement des données à ${hour} :`,
      error
    );
  }
}
...
let dataForSpecificHours = {
  "09h53": { temperature: null, humidity: null },
  "09h54": { temperature: null, humidity: null },
  "09h55": { temperature: null, humidity: null },
};

function checkAndSaveData() {
  const now = new Date();
  const timeKey = `${String(now.getHours()).padStart(2, "0")}h${String(
    now.getMinutes()
  ).padStart(2, "0")}`;

  if (
    dataForSpecificHours[timeKey] &&
    dataForSpecificHours[timeKey].temperature === null
  ) {
    axios
      .post("http://localhost:3002/api/data/save", {
        temperature: sensorData.temperature,
        humidity: sensorData.humidity,
        hour: timeKey,
      })
      .then(() => {
        console.log(
          `Données envoyées et enregistrées avec succès à ${timeKey}`
        );
      })
      .catch((error) => {
        console.error(
          `Erreur lors de l'envoi des données à ${timeKey}:`,
          error
        );
      });
...
async function calculateAndSaveDailyAverages() {
  try {
    ...
    if (dailyData.length > 0) {
      ...
      console.log(
        `Moyenne enregistrée pour ${currentDay}: Température = ${averageTemperature}, Humidité = ${averageHumidity}`
      );
    }
  } catch (error) {
    console.error(
      "Erreur lors du calcul et de l'enregistrement des moyennes:",
      error
    );
  }
}
...
setInterval(() => {
  const now = new Date();
  if (now.getHours() === 9 && now.getMinutes() === 58) {
...
const specificHoursRoutes = ["09h53", "09h54", "09h55"];
  ...
  app.get(`/api/data/${hour}`, async (req, res) => {
    try {
      ...
      if (data.length > 0) {
        ...
        res.status(404).json({
          message: `Aucune donnée trouvée pour l'heure ${hour} aujourd'hui.`,
        });
      }
    } catch (error) {
      console.error(
        `Erreur lors de la récupération des données pour ${hour}:`,
        error
      );
...
app.get("/api/data/daily-averages", async (req, res) => {
  try {
    ...
    const dailyAverage = await DailyAverage.findOne({ jour: getDayOfWeek() })
      .sort({ date: -1 })
      .limit(1);
    ...
    if (dailyAverage) {
      ...
      res
        .status(404)
        .json({ message: "Aucune donnée moyenne trouvée pour aujourd'hui." });
    }
  } catch (error) {
    console.error(
      "Erreur lors de la récupération des moyennes quotidiennes:",
      error
    );
    res.status(500).json({ message: "Erreur serveur" });
  }
